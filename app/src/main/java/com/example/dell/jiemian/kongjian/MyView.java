        ...
        if(data.size()>1){
            for (int i = 1; i < data.size(); i++) {  //依次取出数据进行绘制
                if (data.get(i - 1) != 0&&data.get(i)!=0) {
                    canvas.drawLine(xPoint + (i - 1) * ChartsX, yPoint - data.get(i - 1) * 2.0f, xPoint + i * ChartsX, yPoint - data.get(i) * 2.0f, paint3);
                }
            }
            for(int i=0;i<data.size();i++){
                if(data.get(i)!=0){
                   canvas.drawCircle(xPoint+i*ChartsX,yPoint-data.get(i)*2.0f,2,paint3);
                }
            }
        }
