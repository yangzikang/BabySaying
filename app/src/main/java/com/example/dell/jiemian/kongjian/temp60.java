 ...
 */
public class temp60 extends View {
    ...
    public Handler mh=new Handler(){
        public void handleMessage(android.os.Message msg) {
            if(msg.what==0){                //判断接受消息类型
                temp60.this.invalidate();  //刷新View
            }
        };
    };
    public temp60(Context context, AttributeSet attrs) {
        super(context, attrs);
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setAntiAlias(true);
        canvas.drawText("℃",xPoint-100,yPoint-yLength,paint2);
        ...
        if(data.size()>1){
            ...
            for (int i = 1; i < data.size(); i++) {  //依次取出数据进行绘制
                ...
                if(data.get(i-1)!=0&&data.get(i)!=0) {
                    canvas.drawLine(xPoint + (i - 1) * xScale, secondfloat, xPoint + i * xScale, fourth, paint3);
                }

                if(data.get(i)!=0){
                    canvas.drawCircle(xPoint+i*xScale,fourth,2,paint3);
                }
            }
        }
