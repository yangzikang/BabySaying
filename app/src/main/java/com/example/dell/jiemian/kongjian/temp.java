...
public class temp extends View {
    ...
    //刻度长度
    private int ChartsX=6;
    private int xScale=36;  //35个单位构成一个刻度
    ...
    public List<Float> data=new ArrayList<Float>();   //存放 纵坐标 所描绘的点
    ...
    public void setTime(){
        ...
        for(int j = xLabel.length-1;j>=0;j--){
            hour--;
            if(hour<0){
                hour=23;
            }
            if(minute>9) {
                xLabel[j] = hour + ":" + minute;
            }
            else{
                xLabel[j] = hour + ":0" + minute;
            }
    ...
    protected void onDraw(Canvas canvas) {
        ...
        paint.setAntiAlias(true);
        canvas.drawText("℃",xPoint-60,yPoint-yLength,paint2);
        canvas.drawText("时间",xLength+120,yPoint-20,paint2);
        //绘制Y轴
        ...
        for(int i=0; i*xScale<xLength; i++){
            if(i%4==0) {
                ...
                canvas.drawText(xLabel[i], xPoint + i * xScale, yPoint + 40, paint2);
            }
        }
        canvas.drawText("昨",xPoint-40,yPoint+40,paint2);
        canvas.drawText(getTime(),xPoint+xLength-20,yPoint + 40,paint2);
        //X轴
        ...
        if(data.size()>1){
            for (int i = 1; i < data.size(); i++) {  //依次取出数据进行绘制
                ...
                if (data.get(i - 1) == 0) {
                    ...
                    secondfloat = yPoint - yScale - ((data.get(i - 1) - 32) / 2) * yScale;
                }
                if (data.get(i) == 0) {
                    fourth = yPoint;
                } else {
                    fourth = yPoint - yScale - ((data.get(i) - 32) / 2) * yScale;
                }
                if(data.get(i-1)!=0&&data.get(i)!=0) {
                    canvas.drawLine(xPoint + (i - 1) * ChartsX, secondfloat, xPoint + i * ChartsX, fourth, paint3);
                }
                if(data.get(i)!=0){
                    canvas.drawCircle(xPoint+i*ChartsX,fourth,2,paint3);
                }
            }
        }
